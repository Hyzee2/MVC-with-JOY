 ...
 */
@WebServlet("*.jo")
public class JoyStoryFrontController extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public JoyStoryFrontController() {
		super();
	...
	private void doProcess(HttpServletRequest request, HttpServletResponse response)
		...
		if (command.equals("/JoyStoryList.jo")) {
			action = new JoyListAction(); // Action을 인터페이스하는 BoardListAction 객체 생성. 동적바인딩 사용안하면 BoardListAction action
			try {
				forward = action.execute(request, response); // action을 forward에 담아주고, 맨 아래쪽에서 전송방식과 경로에 맞게 실행된다.
			} catch (Exception e) {
				e.printStackTrace();
			}
		}else if (command.equals("/JoyDetailAction.jo")) {
			action = new JoyDetailAction();
			try {
